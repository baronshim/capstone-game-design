...
```bash
npm run build:client 2>/dev/null; mkdir -p .wrangler; npx wrangler dev --port 8787 > .wrangler/dev.log 2>&1 &
DEV=$!; sleep 8
...
```bash
npm run build:client; mkdir -p .wrangler; npx wrangler dev --port 8787 > .wrangler/dev.log 2>&1 &
DEV=$!; sleep 8
